		...
		case MSG_15002:
			...
			if (newModeList != nil)
			{
				...
				for newItem in newModeList! {
					...
					if (isAdd) {
						if (newItem.uid != nil && newItem.name != nil || newItem.score != nil)
						{
			...
			if (rtmpListStr != "") {
				...
				for item in rtmpList! {
					...
					if (str?.contains("rtmp"))! {
						rtmpInfo.rtmpUrl = (str?.contains("rtmp"))! ? str! : "";
						rtmpInfo.rtmpName = name!;
						resultIpList.append(rtmpInfo);
					}
					else if (str?.contains("http"))!
					{
						rtmpInfo.httpUrl = str!
						let rtmpUrl = rtmpInfo.httpUrl.replace(target: "http", withString: "rtmp");
						var tempList = rtmpUrl.components(separatedBy: ":");
						if (tempList.count > 2) {
							let str2 = tempList[2];
							let preStr = tempList[0] + ":" + tempList[1];
							var tempList2 = str2.components(separatedBy: "/");
							if (tempList2.count > 0) {
								rtmpInfo.rtmpUrl = "\(preStr):\(dataCenterM.roomData.http_flv_testPort)/\(tempList2[1])";
								rtmpInfo.rtmpName = name!;
								rtmpInfo.isHttp = true;
								resultIpList.append(rtmpInfo);
							}
						}
					}
