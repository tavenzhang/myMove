...
+(int)testRtmpConnect:(NSString*)path{
    return [[rtmpHelp instance] RTMP_TestConnect:path];
            ...
            if (strongSelf) {
                ...
                dispatch_sync(dispatch_get_main_queue(), ^{
                  //  NSLog(@"main");
                   ...
                   // NSTimeInterval executionTime = [methodFinish timeIntervalSinceDate:start];
                    NSLog(@"setMovieDecoder Time: %f", executionTime);
                });
            }
        });
    }
...
-(void) close{
    [[RtmpManager shareInstance] setRtmp:nil];
}
- (void) pause
{
    if (!self.playing)
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"closeRtmp" object:NULL];
                    ...
                    if (count > 0) {
                        ...
                        if (_decoder.validVideo) {
                            ...
                            if (delta < -0.1) {
                                ...
                                memset(outData, 0, numFrames * numChannels * sizeof(float));
#ifdef DEBUG
                                LoggerStream(4, @"desync audio (outrun) wait %.4f %.4f", _moviePosition, frame.position);
                                _debugAudioStatus = 1;
...
#ifdef DEBUG
                                LoggerStream(4, @"desync audio (lags) skip %.4f %.4f", _moviePosition, frame.position);
                                _debugAudioStatus = 2;
                                _debugAudioStatusTS = [NSDate date];
#endif
                                continue;
